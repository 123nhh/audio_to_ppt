...
本工具是一个基于 Python 的自动化脚本，用于将本地音频文件（`.mp3`, `.flac`, `.wav`, `.m4a`）批量转换为精美的 PowerPoint (`.pptx`) 演示文稿。
...
打开终端（Windows 下按 `Win+R` 输入 `cmd`），复制并运行以下命令：
...
为了让程序正确运行，请按照以下结构组织文件：

```text
我的工作文件夹/
│
├── audio_to_ppt.py      <-- 将代码保存为这个名字（或者任意 .py 名字）
├── 歌曲1.mp3            <-- 把你要转换的音频文件直接放在这里
├── 歌曲2.flac
├── 歌曲3.m4a
│
└── output/              <-- (程序运行时自动生成，转换好的 PPT 会在这里)
```

**⚠️ 注意：** 音频文件必须和 `.py` 脚本在**同一级目录**下。
...
> **A:** 用记事本打开代码，找到 `batch_process` 函数中的 `max_workers=4`，将数字 `4` 改为你想要的并行数量（建议不超过你的 CPU 核心数）。
